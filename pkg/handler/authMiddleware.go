...
func (handler Handler) AuthMiddleware(ctx *gin.Context) {
	...
	if authHeader == "" {
		newErrorResponse(ctx, http.StatusUnauthorized, fmt.Sprintf(`header "%s" is empty`, authorization))
		return
	}

	authParts := strings.Split(authHeader, " ")
	if len(authParts) != 2 || authParts[0] != bearer {
		newErrorResponse(ctx, http.StatusUnauthorized, fmt.Sprintf(`invalid format of header "%s"`, authorization))
		return
	}
